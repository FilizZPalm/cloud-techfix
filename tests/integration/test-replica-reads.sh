#!/usr/bin/env bash
# tests/integration/test-replica-reads.sh
#
...
#   (proves that new pods created by HPA actually send read queries to the Replica)
#
...
#   3. Takes 100 HPA replica count snapshots during the load test
#   4. Verifies ALL snapshot values are in range [2, 10]
#   5. Records Com_select counter on Replica AFTER the load test
#   6. Verifies Com_select increased (proving reads are hitting the Replica)
#
# Prerequisites:
...
#
# Usage:
#   sudo KUBECONFIG=/etc/rancher/k3s/k3s.yaml bash tests/integration/test-replica-reads.sh
#
# Exit codes:
#   0 — all checks passed
...
SNAPSHOT_COUNT=100
# Interval chosen so that 100 snapshots cover ~3min20s of the 3-min load test
SNAPSHOT_INTERVAL=2
...
# ---------------------------------------------------------------------------
# Helper: get Com_select from MariaDB Replica
# ---------------------------------------------------------------------------
...
for i in $(seq 1 ${SNAPSHOT_COUNT}); do
    ...
    if [[ -z "${replicas}" ]] || ! [[ "${replicas}" =~ ^[0-9]+$ ]]; then
        info "Snapshot ${i}/${SNAPSHOT_COUNT}: could not read replicas (got: '${replicas}'), skipping..."
    ...
    if (( replicas < HPA_MIN || replicas > HPA_MAX )); then
        ...
        # Print progress every 10 snapshots to avoid excessive output
        if (( i % 10 == 0 )); then
...
header "STEP 4: Verify Property 13 — HPA replica count ∈ [${HPA_MIN}, ${HPA_MAX}]"

if [[ ${#SNAPSHOTS[@]} -eq 0 ]]; then
    fail "Property 13: No valid snapshots collected — cannot verify bounds"
else
    if [[ ${OUT_OF_RANGE} -eq 0 ]]; then
        # Compute observed min and max for reporting
        min_val=${SNAPSHOTS[0]}
        max_val=${SNAPSHOTS[0]}
        ...
        pass "Property 13: All ${#SNAPSHOTS[@]} snapshots within [${HPA_MIN}, ${HPA_MAX}] — observed range: [${min_val}, ${max_val}]"
    else
        fail "Property 13: ${OUT_OF_RANGE} snapshot(s) out of range [${HPA_MIN}, ${HPA_MAX}]"
        # Print out-of-range values for debugging
...
# Wait for k6 to complete (runs for 3 minutes total)
if kill -0 "${K6_PID}" 2>/dev/null; then
...
# Small delay to allow in-flight queries to finish
sleep 3
...
# This proves that new pods spawned by HPA actually send reads to the Replica
# ---------------------------------------------------------------------------

header "STEP 6: Verify Com_select increment on MariaDB Replica"
...
if [[ -n "${COM_SELECT_AFTER}" ]] && [[ "${COM_SELECT_AFTER}" =~ ^[0-9]+$ ]]; then
    ...
    if (( DELTA > 0 )); then
        pass "Requirement 12.5: Com_select increased by ${DELTA} — read queries are hitting the Replica"
    else
...
header "REPLICA READS TEST SUMMARY"
echo ""
echo "  Feature: techfix-cloud-migration"
echo "  Property 13: HPA replica count ∈ [2, 10] durante tutto il load test"
echo "  Validates: Requirements 6.4, 12.5"
echo ""
echo "  Total:  ${TOTAL}"
...
if [[ "${FAILED}" -gt 0 ]]; then
    ...
    echo -e "  \033[32m✓ All checks PASSED — Replica receives read traffic during HPA scaling\033[0m"
    echo ""
    exit 0
fi
